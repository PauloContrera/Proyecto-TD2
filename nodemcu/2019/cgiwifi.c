...
int ICACHE_FLASH_ATTR tplWlan(HttpdConnData *connData, char *token, void **arg) {
	...
	if (strcmp(token, "WiFiMode")==0) {
		...
		x=wifi_get_opmode();
		if (x==2) {
			strcpy(buff, "<b>No se pueden buscar redes en este modo</b> Toque <a href=\"setmode.cgi?mode=3\">AQUI</a> para entrar en modo ESTACION+AP.");
		} else {
			strcpy(buff, "Toque <a href=\"setmode.cgi?mode=2\">AQUI</a> para entrar en modo solo AP.");
		}
	}
